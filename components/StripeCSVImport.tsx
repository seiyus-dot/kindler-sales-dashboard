...
export default function StripeCSVImport({ tab, members, onImported }: Props) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch('/api/import-stripe-csv', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ csv, defaultMemberId: memberId || null, dealType: tab }),
    })
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-gray-900">Stripe CSV インポート</h2>
        ...
        <div className="p-6 space-y-5">
          <div className="bg-blue-50 border border-blue-100 rounded p-3 text-sm text-blue-700">
            Stripe ダッシュボード → Billing → Subscriptions → Export で取得した CSV をアップロードしてください。
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-gray-400">インポート後に個別で変更できます</p>
          </div>
          ...
          {loading && (
            <p className="text-sm text-gray-500 text-center py-2">インポート中...</p>
          )}
