...
const KNOWN_FORMATS: {
  ...
  {
    // Utage 個別相談会申込者CSVエクスポート（面談日程・氏名フォーマット）
    matchHeaders: ['申込日時', '面談日程', '担当者', '氏名', 'フリガナ', '参加URL', '参加状況', '成約状況', 'キャンセル理由'],
    targetTable: 'aicamp_consultations',
    reason: '個別相談会申込者CSV（Utage出力フォーマット）（ルールベース判定）',
    mappings: {
      '申込日時':              'applied_at',
      '面談日程':              'consultation_date',
      '担当者':                'member_id',
      '氏名':                  'name',
      'フリガナ':              null,
      'メールアドレス':        'email',
...
export async function POST(req: NextRequest) {
  ...
  const { headers, sampleRows, allRows, defaultMemberId, defaultServiceType, members, forceTable } = await req.json()
...
`
  ...
  for (const row of allRows) {
    ...
    for (const [srcCol, dstField] of Object.entries(mappings)) {
      ...
      // 担当者名 → UUID変換（解決できなくても必ずキーを設定してカラム順を保持）
      if (field === 'member_id') {
        const resolved = val ? resolveMemberId(String(val)) : null
        mapped.member_id = resolved ?? defaultMemberId ?? null
        if (resolved) memberResolved = true
        continue
      }
    ...
    // aicamp_consultations: service_type が未設定の場合はデフォルト値を適用
    if (targetTable === 'aicamp_consultations' && !mapped.service_type && defaultServiceType) {
      mapped.service_type = defaultServiceType
    }
